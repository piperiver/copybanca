...
@section('content')
@inject('Users', 'App\Http\Controllers\UsersController')
    ...
    <div class="col-md-12 col-sm-12">
        <div class="portlet box main-color">
            <div class="portlet-title">
                ...
                <div class="actions">
                    <select id="slUserAg" name="slUserAg" class="form-control select2 circle">
                        @php($Lista = $Users->listarUsers())
                        <option value=""></option>
                        @for($i=0; $i < count($Lista); $i++)
                            <option value="{{$Lista[$i]['id']}}">{{$Lista[$i]['nombre'].' '.$Lista[$i]['apellido']}}</option>
                        @endfor
                    </select>
                </div>
            </div>
...
<input type="hidden" name="eventos" id="eventos" value="{{$datos}}"/>
    ...
    <!-- Modal content-->
        ...
        <div class="modal-body">
            ...
            <div class="control-group">
                ...
                <div class="field desc">
                    <select id="slUser" class="form-control select2 circle">
                        @php($Lista = $Users->listarUsers())
                        <option value=""></option>
                        @for($i=0; $i < count($Lista); $i++)
                            <option value="{{$Lista[$i]['id']}}">{{$Lista[$i]['nombre'].' '.$Lista[$i]['apellido']}}</option>
            ...
            <div class="modal-footer">
                <button class="btn" data-dismiss="modal" aria-hidden="true">Cerrrar</button>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="control-group">
                    <label class="control-label" for="inputPatient">Usuario:</label>
                    <div class="field desc">
                        <input class="form-control" id="modalUsuario" name="modalUsuario" placeholder="Usuario" type="text" value="">
                    </div>
                </div>
                <div class="control-group">
                    <label class="control-label" for="inputPatient">Lugar:</label>
...
<script>
    $("#slUser").change(function(){
       $('#usuario').val($('#slUser').val()); 
    });
    
    $("#slUserAg").change(function(){
       renderFilter(); 
    });
