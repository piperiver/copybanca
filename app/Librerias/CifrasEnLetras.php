...
class CifrasEnLetras {
    ...
    protected static function convertirCentenas($centenas, $genero = 'neutro') {
        ...
        if ($genero == 'femenino')
            $resultado = str_replace('iento', 'ienta', $resultado);
        return $resultado;
    }
    ...
    public static function convertirDosCifras($cifras, $genero = 'neutro') {
        ...
        if ($cifras < 30)
            return self::convertirUnidades($cifras, $genero);
        elseif ($unidad == 0)
            return self::$listaDecenas[$decena];
        else
            return self::$listaDecenas[$decena] . ' y ' . self::convertirUnidades($unidad, $genero);
    }
    ...
    public static function convertirTresCifras($cifras, $genero = 'neutro') {
        ...
        if ($cifras < 100)
            return self::convertirDosCifras($cifras, $genero);
        elseif ($decenas_y_unidades == 0)
            return self::convertirCentenas($centenas, $genero);
        elseif ($centenas == 1)
            return 'ciento ' . self::convertirDosCifras($decenas_y_unidades, $genero);
        else
            return self::convertirCentenas($centenas, $genero) . ' ' . self::convertirDosCifras($decenas_y_unidades, $genero);
    }
    ...
    public static function convertirSeisCifras($cifras, $genero = 'neutro') {
        ...
        if ($grupo_miles == 0)
            return self::convertirTresCifras($primer_millar, $genero);
        elseif ($grupo_miles == 1) {
            if ($primer_millar == 0)
                return 'mil';
            else
                return 'mil ' . self::convertirTresCifras($primer_millar, $genero);
        }
        elseif ($primer_millar == 0)
            return self::convertirTresCifras($grupo_miles, $genero_miles) . ' mil';
        else
            return self::convertirTresCifras($grupo_miles, $genero_miles) . ' mil ' . self::convertirTresCifras($primer_millar, $genero);
    }
    ...
    public static function convertirCifrasEnLetras($cifras, $genero = 'neutro', $separadorGruposSeisCifras = null) {
        ...
        if ($separadorGruposSeisCifras == null)
            $separadorGruposSeisCifras = self::$SEPARADOR_SEIS_CIFRAS;

        // Comprobación
        if ($numeroCifras == 0)
            return self::PREFIJO_ERROR . 'No hay ningún número';
        for ($indiceCifra = 0; $indiceCifra < $numeroCifras; $indiceCifra++) {
            ...
            if (!$esDecimal)
                return self::PREFIJO_ERROR . 'Uno de los caracteres no es una cifra decimal';
        }
        if ($numeroCifras > 126)
            return self::PREFIJO_ERROR . 'El número es demasiado grande ya que tiene más de 126 cifras';
        ...
        for ($indiceGrupo = 0; $indiceGrupo < $numeroGruposSeisCifras * 6; $indiceGrupo += 6) {
            ...
            if ($seisCifras != 0) {
                if (count($resultado) > 0)
                    $resultado[] = $separadorGruposSeisCifras;
        ...
        // Finalización
        if (count($resultado) == 0)
            $resultado[] = self::$listaUnidades[0];
    ...
    $cifras, $numeroDecimales = -1, $palabraEntera = '', $palabraEnteraPlural = '', $esFemeninaPalabraEntera = false, $palabraDecimal = '', $palabraDecimalPlural = '', $esFemeninaPalabraDecimal = false) {
        // Argumentos
        ...
        $parteEntera = self::procesarEnLetras($cifrasEntera, $palabraEntera, $palabraEnteraPlural, $esFemeninaPalabraEntera);
        if (self::empiezaPor($parteEntera, self::PREFIJO_ERROR))
            return $parteEntera;
        $resultado[] = $parteEntera;

        if ($cifrasDecimal != "") {
            $parteDecimal = self::procesarEnLetras($cifrasDecimal, $palabraDecimal, $palabraDecimalPlural, $esFemeninaPalabraDecimal);
            if (self::empiezaPor($parteDecimal, self::PREFIJO_ERROR))
                return $parteDecimal;
            $resultado[] = " con ";
    ...
    public static function convertirEurosEnLetras($euros, $numeroDecimales = 2) {
        return self::convertirNumeroEnLetras($euros, $numeroDecimales, "euro", "euros", false, "céntimo", "céntimos", false);
    }
    ...
    private static function procesarEnLetras($cifras, $palabraSingular, $palabraPlural, $esFemenina) {
        ...
        if ($esFemenina)
            $genero = "femenino";
        else if ($palabraSingular == "")
            $genero = "masculino";

        // Letras
        $letras = self::convertirCifrasEnLetras($cifras, $genero);
        if (self::empiezaPor($letras, self::PREFIJO_ERROR))
            return $letras;
